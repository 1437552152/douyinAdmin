<template>
  <div>
     <Card>
      ...
      <Modal 
        class="passCoinModal" width="400" v-model="ifPassCoin"  @on-ok="confrimPass">
              	<Form :model="signModal" :label-position="'left'" :label-width='80'>
					<FormItem label='转币方式' prop="unit">
						<Select v-model="mode" >
							<Option v-for=" (coin,index) in allCoinArr" :value="coin.value" :key="index">{{ coin.label }}</Option>
						</Select>
					</FormItem>
                    <FormItem label='提现数量' prop="unit">
						<Input v-model="userInfo.totalAmount"  disabled></Input>
					</FormItem>
                    <FormItem label='提现币种' >
						<Input v-model="userInfo.unit"  disabled></Input>
					</FormItem>
                     <FormItem label='手续费'>
						<Input v-model="userInfo.fee "  disabled></Input>
					</FormItem>
                     <FormItem label='到账数量：' prop="unit">
						<Input v-model="userInfo.arrivedAmount"  disabled></Input>
					</FormItem>
					<FormItem label="交易流水号" prop="amount" v-if='mode==1'>
						<Input v-model="transactionNum" 
						></Input>
					</FormItem>
...
import { auditPass, auditNoPass, withdrawManage, perTradeAll, passCoinByOne ,autuWithdraw} from '@/service/getData';
...
export default{
        ...
        columns_first: [
				...
				},
				{
                    ...
                    render: (h, obj) => {
						...
						'活动奖励', '推广奖励', '分红', '投票', '人工充值', '配对',"缴纳商家认证保证金","退回商家认证保证金" ];      
						return h('span', {
						}, arr[type])
					}
				},
    ...
    methods: {
     ...
     changePage (pageIndex) {
			perTradeAll({ pageNo: pageIndex, pageSize: 10, memberId: this.userInfo.memberId})
			.then( res => {
    ...
    ifPass (bol) {
     ...
     refreshPage() {
       ...
       perTradeAll({ pageNo: 1, pageSize: 10,memberId: this.userInfo.memberId })
       .then( res => {
         if(!res.code)
         this.trade_data = res.data.content;
