					...
					<div class="body">
						...
						<Row v-if="!!totalVolume.tabelArr.length">  
							<Row class="info"  v-for="tabelInfo in totalVolume.tabelArr" :key="tabelInfo.unit">
								...
								<Col span="7" offset="1">{{ tabelInfo.num ? tabelInfo.num : 0 | numLen }}</Col>
								<Col span="7" offset="1">{{ tabelInfo.money ? tabelInfo.money : 0 | numLen }}</Col>
							</Row>
						</Row>
			...
			.then(res => {
				if(!res.code) {
					...
					res.data.forEach(item => {
						...
						if(saveIndex===-1) {
							if(item.type==='OTC_MONEY' || item.type==='EXCHANGE_BASE') {
								this.totalVolume.tabelArr.push({ unit: item.unit, money: item.amount })
							}else if(item.type==='OTC_NUM'  || item.type==='EXCHANGE_COIN') {
