<template>
	<Card class="businessDetail">
		...
		<Row>
			<Col span="6">
				vip审核状态：<span class="status">{{ userInfo.vipApplyStatus | filterStatus }}</span>
			</Col>
			<Col span="6">
				真实姓名：<span>{{ userInfo.member.realName }}</span>
			</Col>
			<Col span="6">
				保证金：<span>{{ userInfo.info.amount + userInfo.info.coinSymbol }}</span>
			</Col>
		</Row>
		<Row>
			<Col span="6">
...
<script>
import { vipApplyDetail } from '@/service/getData'
export default {
	name: 'BusinessDetail',
	data() {
		return {
	...
	methods: {
		...
		refreshPage() {
			this.ifLoading = true;
			vipApplyDetail({id: this.$route.query.id})
			.then(res => {
				if(!res.code){
