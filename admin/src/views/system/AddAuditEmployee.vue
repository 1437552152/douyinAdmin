            ...
            <FormItem label="真实姓名：" prop="realName">
              <Input type="text" v-model="formWrapperObj.realName"></Input>
               ...
               <FormItem prop="country" label="国家">
                ...
                <Select v-model="formWrapperObj.country" placeholder='请选择国家'  @on-change="onAreaChange">
             ...
             <FormItem label="联系电话：" prop="mobilePhone">
                        <Input v-if="changeActive == 0" type="text" v-model="formWrapperObj.mobilePhone" >
                            <span slot="prepend">+{{formWrapperObj.areaCode}}</span>
                        </Input>
                        <Input v-else type="text" v-model="formWrapperObj.mobilePhone" >
                    </Input>
                    
                    <!-- <Select v-model="formWrapperObj.areaCode" slot="prepend" style="width: 57px;position:absolute;">
...
export default {
  ...
  methods: {
    ...
    getArea(){
         axios.post(BASEURL+'admin/system/employee/allCountry').then(res => {
            if(res.data.code==0){
    ...
    onAreaChange(value){
            for(var i=0;i<this.areas.length;i++){
                if(this.areas[i].zhName == value){
                    this.formWrapperObj.areaCode = this.areas[i].areaCode;
                }
            }
        },
